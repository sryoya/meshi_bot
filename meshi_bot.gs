...
function doPost(e) {
  var token = '';
  var bot_name = "飯ぼっと";
  var bot_icon = "http://www.sakana-center.com/wp-content/uploads/2013/01/3e1493954cf1178dda32d2730c568a66.jpg";
  
  var app = SlackApp.create(token);
  var channelId = e.parameter.channel_id
  var common_query= "http://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=984e92505d475bc4&keyword=";
  var text = e.parameter.text.substr(5);
  var url = common_query + text 
  
  	//var url = "https://www.google.co.jp/"
 	//var url = "http://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=984e92505d475bc4&keyword=東京%20ディナー"
 	// GETリクエスト
  	var response = UrlFetchApp.fetch(url).getContentText("UTF-8");
	// HTMLの結果を取得（引数のcharsetは設定したほうが良い）
   var doc = Parser.data(response).from('<urls><pc>').to('</pc>').build();
  
 	//var content = response.getContentText();
 	//var dom = XmlService.parse(content);
 	//var content = getElementsByTagName(dom, 'results');
  
  
 	//var request = new XMLHttpRequest();
 	//var request = new XmlRpcRequest("http://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=984e92505d475bc4&keyword=日本橋", "GET");
   ...
   return app.postMessage(channelId, doc , {
   username: bot_name,
   icon_url: bot_icon
  });
}
